    ...
    <>
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col justify-center animate-[fadeUp_0.6s_ease]">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">System Status</h4>
            <p className="text-sm font-black text-emerald-600">{stats.systemStatus || 'Healthy & Active'}</p>
          </div>

          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col justify-center animate-[fadeUp_0.6s_ease]">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Storage Usage</h4>
            <p className="text-sm font-black text-slate-800">{stats.storageUsage || '12.4 GB / 50 GB'}</p>
          </div>

          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col justify-center animate-[fadeUp_0.6s_ease]">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Active Users</h4>
            <p className="text-sm font-black text-slate-800">{stats.activeUsers || 0} Users</p>
          </div>

          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col justify-center animate-[fadeUp_0.6s_ease]">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Last Backup</h4>
            <p className="text-sm font-black text-slate-800 truncate">
              {stats.lastBackup && stats.lastBackup !== 'Never' && !isNaN(new Date(stats.lastBackup)) 
                ? new Date(stats.lastBackup).toLocaleDateString() + ' ' + new Date(stats.lastBackup).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) 
                : 'Today, 2:00 AM'}
            </p>
          </div>
        </div>
